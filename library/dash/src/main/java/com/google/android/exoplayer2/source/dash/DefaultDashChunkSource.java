...
//    Timber.d("COMM:1 getNextChunk| playbackPosition(s): %f; previous MediaChunk: %s", playbackPositionUs / 1000000f, previous);
    ...
    Timber.d("··getNextChunk| selected RepresentationHolder %s", representationHolder);
  ...
  public void onChunkLoadCompleted(Chunk chunk) {
    if (chunk instanceof InitializationChunk) {
      ...
      if (representationHolder.segmentIndex == null) {
        ...
        if (seekMap != null) {
          ChunkIndex chunkIndex = (ChunkIndex) seekMap;
          representationHolder.segmentIndex = new DashWrappingSegmentIndex(chunkIndex);
          trackSelection.dashSegmentIndexLoaded(trackIndex, chunkIndex);
        }
      }
    }
  }
